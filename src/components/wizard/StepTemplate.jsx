    ...
    return (
      <div>
        ...
        <input
          ...
          className="w-full max-w-md bg-gray-800 border border-gray-600 rounded px-3 py-2 text-gray-100 focus:outline-none focus:border-wod-silver"
        />
      </div>
    )
  }
        ...
        {group.options.map(opt => (
          <button
            ...
            className={`text-left p-3 rounded-lg border transition-all ${
              selected === opt.id
                ? 'border-wod-silver bg-gray-800 ring-1 ring-wod-silver'
          ...
          >
            ...
            {opt.hint && <div className="text-xs text-wod-red mt-1">{opt.hint}</div>}
          </button>
        ))}
      </div>
    </div>
...
export default function StepTemplate({ lineData, template, onUpdate }) {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="wod-heading text-2xl font-bold mb-2">Supernatural Template</h2>
        <p className="text-wod-stone mb-6">Choose your {lineData.name} origin and affiliation.</p>
      </div>
